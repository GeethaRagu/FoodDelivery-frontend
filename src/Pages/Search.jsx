  ...
  return (
    <div>
      ...
      <form onSubmit={handleSearch}>
        ...
        />
        <button type="submit" className="w-25 h-15 text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-2xl text-sm px-5 py-2.5 text-center mb-2 dark:focus:ring-yellow-900 ">Search</button>
        </div>
      </form>
      ...
      <div className="grid md:grid-cols-4 md:gap-4  sm:grid-cols-2 grid-cols-1 gap-2">
        {results.map((element, index) => {
          return <FoodItem element={element} index={index} />;
        })}
      </div>}
